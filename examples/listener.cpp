...
std::expected<Socket, std::runtime_error> make_listener() {
    auto address_candidates = Socket::get_address_info("", PORT, Socket::Family::IPV6, Socket::Type::DGRAM);
    ...
    for (auto &address_info: *address_candidates) {
        ...
        if (socket) {
            auto expected = socket->set_option(SO_REUSEPORT, 1)
...
int main() {
    ...
    std::cout << "listener: waiting for data " << socket->get_address()->get_as_inet()->get_as_string() << std::endl;
    ...
    std::cout << "listener: packet contains \"" << std::string(buffer.begin(), buffer.end()) << '\"' << std::endl;
}