...
std::expected<Socket, std::exception> make_client(std::string_view hostname) {
    auto address_candidates = Socket::get_address_info(hostname, SERVER_PORT, Socket::Family::UNSPECIFIED, Socket::Type::STREAM);
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "client: connecting to " << socket->get_peer_address()->get_as_inet()->get_as_string() << "..." << std::endl;
    ...
    std::cout << "client: received: \"" << std::string(received->begin(), received->end()) << '\"' << std::endl;
}
