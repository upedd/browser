...
std::expected<std::pair<Socket::AddressInformation, Socket>, std::exception> make_listener(std::string_view hostname) {
    auto address_candidates = Socket::get_address_info(hostname, SERVER_PORT, Socket::Family::IPV6, Socket::Type::DGRAM);
...
int main(int argc, char *argv[])
{
    ...
    if (!address_and_socket) {
        std::cerr << "talker: failed to create socket: " << address_and_socket.error().what() << std::endl;
        return -1;
    }
