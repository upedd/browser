...
std::expected<Socket, std::runtime_error> make_server() {
    auto address_candidates = Socket::get_address_info("", PORT, Socket::Family::IPV4, Socket::Type::STREAM);
...
int main() {
    ...
    if (!socket) {
        std::cout << "server: couldn't create socket for server: " << socket.error().what() << std::endl;
        return -1;
    }
